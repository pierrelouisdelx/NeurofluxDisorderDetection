...
class MRIPreprocessor:
    ...
    def detect_outliers(self, image_paths, labels, threshold=2.0):
        """
        ...
        if len(outlier_indices) > 0:
            fig, axes = plt.subplots(
                min(10, len(outlier_indices)),
                2,
                figsize=(12, 2 * min(10, len(outlier_indices))),
            )
            ...
            for i, idx in enumerate(outlier_indices[:10]):
                try:
                    ...
                    axes[i, 1].bar(
                        ["Intensity", "Contrast", "SNR", "Entropy"],
                        z_scores[idx],
                        color="red",
                    )
            ...
            outlier_df = pd.DataFrame(
                {
                    ...
                    "intensity": [
                        self.stats["intensity"]["values"][i] for i in outlier_indices
                    ],
                    "contrast": [
                        self.stats["contrast"]["values"][i] for i in outlier_indices
                    ],
                    "snr": [self.stats["snr"]["values"][i] for i in outlier_indices],
                    "entropy": [
                        self.stats["entropy"]["values"][i] for i in outlier_indices
                    ],
                    "max_z_score": max_z_scores[outlier_indices],
                }
            )
            outlier_df.to_csv(
                os.path.join(self.output_path, "outliers.csv"), index=False
            )

        print(
            f"Detected {len(outlier_indices)} outliers out of {len(image_paths)} images"
        )
        return outlier_indices

    def visualize_dataset_tsne(
        self, image_paths, labels, perplexity=30, save_dir="tsne_results"
    ):
        """
        ...
        if self.outliers:
            plt.scatter(
                tsne_result[self.outliers, 0],
                tsne_result[self.outliers, 1],
                marker="x",
                c="black",
                s=100,
                label="Outliers",
    ...
    def _plot_histograms(self, save_dir):
        ...
        # Intensity histogram
        axes[0, 0].hist(
            self.stats["intensity"]["values"], bins=50, color="blue", alpha=0.7
        )
        axes[0, 0].axvline(self.stats["intensity"]["mean"], color="red", linestyle="--")
        axes[0, 0].set_title(
            f"Intensity Distribution (Mean: {self.stats['intensity']['mean']:.2f})"
        )

        # Contrast histogram
        axes[0, 1].hist(
            self.stats["contrast"]["values"], bins=50, color="green", alpha=0.7
        )
        axes[0, 1].axvline(self.stats["contrast"]["mean"], color="red", linestyle="--")
        axes[0, 1].set_title(
            f"Contrast Distribution (Mean: {self.stats['contrast']['mean']:.2f})"
        )
        ...
        axes[1, 0].set_title(
            f"SNR Distribution (Mean: {self.stats['snr']['mean']:.2f})"
        )

        # Entropy histogram
        axes[1, 1].hist(
            self.stats["entropy"]["values"], bins=50, color="orange", alpha=0.7
        )
        axes[1, 1].axvline(self.stats["entropy"]["mean"], color="red", linestyle="--")
        axes[1, 1].set_title(
            f"Entropy Distribution (Mean: {self.stats['entropy']['mean']:.2f})"
        )
