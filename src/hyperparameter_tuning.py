...
def objective(
    ...
    model = ModelFactory(
        model_name, len(class_names), hyperparams=hyperparams
    ).get_model()
    model.to(device)
    ...
    model = train_model(
        ...
        model_save_path=os.path.join(
            output_dir, f"{model_name}_trial_{trial.number}.pth"
        ),
...
def optimize_hyperparameters(
    model_name, dataset_config_path, output_dir="output/hyperparameter_tuning"
):
    """
